...
@Tag("smoke")
class BybitMarketStatsClientSmokeTest {

    @Test
    void fetch_spot_and_perp_BTCUSDT_havePositiveVolume() {
        assumeOnline();

        var client = new BybitMarketStatsClient();

        var spot = new MarketRef("BTC", "BYBIT", "SPOT", "BTCUSDT");
        var perp = new MarketRef("BTC", "BYBIT", "PERP", "BTCUSDT");
